    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            this.partitioner = config.getConfiguredInstance(
                    ...
                    Partitioner.class,
                    Collections.singletonMap(ProducerConfig.CLIENT_ID_CONFIG, clientId));
            // 重试时间间隔 retry.backoff.ms 默认100ms
            ...
            /*
                  maxRequestSize 生产者往服务端发送消息到时候，规定一条消息最大多大？ 🔥
                  如果你超过了这个规定消息的大小，你的消息就不能发送过去。
                  默认是 1M，这个值偏小，在生产环境中，我们需要修改这个值。
                  经验是 10M。但是大家也可以根据自己公司的情况来。
             */
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);
            //  缓存大小，默认为 32M，一般够用
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            //  压缩格式：提高系统吞吐量。 一次发送出去到消息就更多，生产者也会消耗更多到 CPU
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));
            // 最大阻塞时间，默认60s
            this.maxBlockTimeMs = config.getLong(ProducerConfig.MAX_BLOCK_MS_CONFIG);
            // 发送消息上报 成功或失败的最大时间，也就是最终必须给一个反馈
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    ...
                    lingerMs(config),   // 发送延迟时间 如果没有延迟，16KB批次大小可能只有1KB的内容就被发送了
                    retryBackoffMs,
                    deliveryTimeoutMs,
                    metrics,
